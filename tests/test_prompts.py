...
with open(os.path.join(script_dir, 'results', 'results.csv'), 'w', newline='') as file:
    writer = csv.DictWriter(file, fieldnames=['prompt_id', 'request_id', 'response', 'cost'])
    writer.writeheader()
    ...
    for prompt in prompts:
        for request in requests:
            ...
            writer.writerow({
                ...
                'response': response['choices'][0]['message']['content'],  # Assuming you want the first response message
